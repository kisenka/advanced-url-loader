...
module.exports = function (content) {
  ...
  var handlers = extend({}, defaultHandlers, (this.options && this.options.advancedUrl && this.options.advancedUrl.handlers) || {});
  var handler = mimetype in handlers ? handlers[mimetype] : null;

  if (limit <= 0 || content.length < limit) {
    var result = handler
      ? handler(content)
      : 'data:' + (mimetype ? mimetype + ';' : '') + 'base64,' + content.toString('base64');
